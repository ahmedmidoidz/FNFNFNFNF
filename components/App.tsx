...
import { PlusIcon, MenuIcon, SparklesIcon, BotIcon, WalletIcon, EyeIcon, EyeOffIcon } from './Icons';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      try {
          ...
          const glassBlur = settings.backgroundBlur !== undefined ? settings.backgroundBlur : 16;
          // Wallpaper blur level
          const wallpaperBlur = settings.wallpaperBlur !== undefined ? settings.wallpaperBlur : 0;

          if (settings.cardStyle === 'solid') {
              // Dark mode uses Stone 900 (#1C1917), Light mode uses Cream (#FFFFFF)
              root.style.setProperty('--glass-bg', financeData.darkMode ? '#1C1917' : '#FFFFFF');
              root.style.setProperty('--glass-border', 'transparent');
              root.style.setProperty('--glass-blur', '0px');
              root.style.setProperty('--glass-blur-heavy', '0px');
              root.style.setProperty('--glass-blur-input', '0px');
          } else if (settings.cardStyle === 'outline') {
              root.style.setProperty('--glass-bg', 'transparent');
              root.style.setProperty('--glass-border', financeData.darkMode ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.15)');
              root.style.setProperty('--glass-blur', '0px');
              root.style.setProperty('--glass-blur-heavy', '0px');
              root.style.setProperty('--glass-blur-input', '0px');
          } else {
              // Default Glass (Refined for Warm/Brown Mode)
              // Dark: Warm Stone 800 with opacity (Brownish) | Light: High opacity White
              root.style.setProperty('--glass-bg', financeData.darkMode ? 'rgba(41, 37, 36, 0.7)' : 'rgba(255, 255, 255, 0.8)');
              root.style.setProperty('--glass-border', financeData.darkMode ? 'rgba(255, 255, 255, 0.06)' : 'rgba(224, 222, 215, 0.6)');
              
              // Dynamic Blur Calculation for cards
              root.style.setProperty('--glass-blur', `${glassBlur}px`);
              root.style.setProperty('--glass-blur-heavy', `${Math.min(glassBlur * 1.5, 50)}px`); 
              root.style.setProperty('--glass-blur-input', `${Math.max(glassBlur * 0.6, 2)}px`);
          }

          // 3. Handle Background Image & Blur Overlay
          const wallpaperEl = document.getElementById('app-wallpaper');
          const overlayEl = document.getElementById('wallpaper-overlay');
          const blobsEl = document.querySelector('.blob-bg') as HTMLElement;

          if (settings.backgroundImage) {
              // Reset body background to avoid conflict
              document.body.style.backgroundImage = '';

              if (wallpaperEl) {
                  wallpaperEl.style.backgroundImage = `url(${settings.backgroundImage})`;
                  wallpaperEl.style.display = 'block';
              }
              
              // Show and set blur on the dedicated overlay div
              if (overlayEl) {
                  overlayEl.style.display = 'block';
                  overlayEl.style.backdropFilter = `blur(${wallpaperBlur}px)`;
                  (overlayEl.style as any).webkitBackdropFilter = `blur(${wallpaperBlur}px)`;
              }
              
              if(blobsEl) blobsEl.style.display = 'none';
          } else {
              if (wallpaperEl) wallpaperEl.style.display = 'none';
              if (overlayEl) overlayEl.style.display = 'none';
              
              document.body.style.backgroundImage = '';
              if(blobsEl) blobsEl.style.display = 'block';
          }
      } catch (e) {
      ...
      financeData.settings.wallpaperBlur,
      financeData.darkMode
  ]);
